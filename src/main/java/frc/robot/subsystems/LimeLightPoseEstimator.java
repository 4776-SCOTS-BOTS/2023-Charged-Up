...
public class LimeLightPoseEstimator extends SubsystemBase {
  ...
  public TimestampedBotPose3d[] getBotPose3d() {
    ...
    timestampedPoses3D[1].timestamp =   Timer.getFPGATimestamp() - (result[6]/1000.0);
    botTran3d = new Translation3d(result[0], result[1], result[2]);
    botRot3d = new Rotation3d(result[3], result[4], result[5]);
    timestampedPoses3D[1].pose3d = new Pose3d(botTran3d, botRot3d);
    timestampedPoses3D[1].tagID = getAprilTagID();
    ...
    //   timestampedPoses3D[i].timestamp =   Timer.getFPGATimestamp() - (poseValue[6]/1000.0);
